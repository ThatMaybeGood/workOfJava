...
class InpatientPrepaymentController {
    ...
    handleDimensionChange(e) {
        ...
        document.querySelectorAll('.date-axis-header').forEach(th => {
            th.textContent = this.filter.dimension === 'month' ? '月份' : '日期';
        });
        this.resetDatePicker();
        this.summaryTableState.currentPage = 1;
        this.incomeTableState.currentPage = 1;
    ...
    async loadIncomeOverview() {
        try {
            const body = await ReportAPI.getInpatientPrepaymentOverview({ ...this.filter, type: 'income_count' });
    ...
    async loadRefundOverview() {
        try {
            const body = await ReportAPI.getInpatientPrepaymentOverview({ ...this.filter, type: 'refund_count' });
            if (body && body.prepaymentCount !== undefined) {
