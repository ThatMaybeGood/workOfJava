...
CREATE TABLE tr_fc_weather (
    ...
    weather_source  VARCHAR2(20),                       -- 来源(人工登记/接口同步)
    create_time     DATE            DEFAULT SYSDATE,       -- 创建时间
    update_time     DATE            DEFAULT SYSDATE,       -- 更新时间
    ext1            VARCHAR2(500),                        -- 扩展字段1
    ext2            VARCHAR2(500),                        -- 扩展字段2
...
COMMENT ON COLUMN tr_fc_weather.weather_coef IS '天气出勤系数(空按1处理,保存时按近30天同天气日期爽约退号率自动计算:1-率)';
COMMENT ON COLUMN tr_fc_weather.weather_source IS '来源(人工登记/接口同步)';
