...
public class HisDataServiceImpl implements HisDataService {
    ...
    @Override
    public List<HisIncomeDTO> findByDate(LocalDate reportDate) {
        try {
            // 假设API返回的是HisData数组
            ResponseEntity<HisIncomeDTO[]> response = restTemplate.getForEntity(HIS_DATA_URL, HisIncomeDTO[].class, reportDate);
