...
import com.example.messagedataservice.server.HisOperatorService;
...
public class HisOperatorServiceImpl implements HisOperatorService {
    ...
    @Override
    public List<HisOperator> findData() {
        try {
            // 假设API返回的是HisOperator数组
            ResponseEntity<HisOperator[]> response = restTemplate.getForEntity(OPERATOR_URL, HisOperator[].class);
